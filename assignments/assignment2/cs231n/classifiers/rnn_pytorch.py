...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        word_emb = word_embedding_forward(captions_in, W_embed)
        
        if self.cell_type == "rnn":
            h_next = rnn_forward(word_emb, h0, Wx, Wh, b)
        else:
            h_next = lstm_forward(word_emb, h0, Wx, Wh, b)
        
        scores = temporal_affine_forward(h_next, W_vocab, b_vocab)
        
        loss = temporal_softmax_loss(scores, captions_out, mask, verbose=False)
    ...
    def sample(self, features, max_length=30):
        ...
        # Start token for all sequences in the batch, shape (N,)
        word = self._start * torch.ones(N, dtype=torch.long)
        
        for t in range(max_length):
            
            word_emb = word_embedding_forward(word, W_embed)
            
            if self.cell_type == "rnn":
                h = rnn_step_forward(word_emb, h, Wx, Wh, b)
            else:
                h = lstm_step_forward(word_emb, h0, Wx, Wh, b)
            
            scores = affine_forward(h, W_vocab, b_vocab)
            
            # (4) Pick highest scoring word for each sequence in batch
