...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        # l(k, k+N)
        numerator_left = torch.exp(sim(z_k, z_k_N) / tau)
        denominator_left = torch.sum(
            torch.stack([torch.exp(sim(z_k, out[m]) / tau) for m in range(2*N) if m != k])
        )
        lk_kN = -torch.log(numerator_left / denominator_left)
    
        # l(k+N, k)
        numerator_right = torch.exp(sim(z_k_N, z_k) / tau)
        denominator_right = torch.sum(
            torch.stack([torch.exp(sim(z_k_N, out[m]) / tau) for m in range(2*N) if m != k+N])
        )
        lkN_k = -torch.log(numerator_right / denominator_right)
    
        total_loss += lk_kN + lkN_k
...
def compute_sim_matrix(out):
    ...
    # Normalized dot product between all pairs = matrix multiplication
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    # Step 2: Positive pair similarities — out_left[k] pairs with out_right[k]
    # In the concatenated matrix, positive pairs are at indices (k, k+N) and (k+N, k)
    pos_sim = sim_positive_pairs(out_left, out_right)  # [N, 1]
    pos_sim = torch.cat([pos_sim, pos_sim], dim=0).squeeze(1)  # [2*N]
    
    # Step 3: Numerator
    numerator = torch.exp(pos_sim / tau)  # [2*N]
