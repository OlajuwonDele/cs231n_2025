...
def get_similarity_no_loop(text_features, image_features):
    ...
    image_features  = image_features  / image_features.norm(dim=-1, keepdim=True)   # (M, D)
...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    # Tokenize the class strings → (C, context_length) token tensor
    text_tokens = clip.tokenize(class_texts).to(device)   # (C, L)

    with torch.no_grad():
        text_features = clip_model.encode_text(text_tokens)    # (C, D)

    # ------------------------------------------------------------------ #
    # 2. Encode each image and classify                                   #
    # ------------------------------------------------------------------ #
    for image in images:
        # Convert HxWxC uint8 NumPy array → PIL Image for preprocessing
        pil_image = Image.fromarray(image)

        # clip_preprocess returns a (3, H, W) tensor; add batch dim → (1, 3, H, W)
        image_tensor = clip_preprocess(pil_image).unsqueeze(0).to(device)  # (1, D)

        with torch.no_grad():
            image_features = clip_model.encode_image(image_tensor)         # (1, D)

        # ------------------------------------------------------------------ #
        # 3. Compute cosine similarity and predict                           #
        # ------------------------------------------------------------------ #
        # get_similarity_no_loop(text_features, image_features)
        # text_features : (C, D)  → N dimension
        # image_features: (1, D)  → M dimension
        # output shape  : (C, 1)
        similarity = get_similarity_no_loop(text_features, image_features)  # (C, 1)

        # Class with highest similarity to this image
        best_class_idx = similarity.squeeze().argmax().item()               # scalar
        pred_classes.append(class_texts[best_class_idx])
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        image_batch = torch.stack([ clip_preprocess(Image.fromarray(img)) for img in images ]).to(device)
        image_features = clip_model.encode_image(image_batch)   # (N, D)
        # L2-normalise so retrieval reduces to a plain dot product
        self.image_features = image_features / image_features.norm(dim=-1, keepdim=True)  # (N, D)
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        text_tokens   = clip.tokenize([query]).to(self.device)                          # self.device
        ...
        top_k_indices = similarity.topk(k).indices.cpu().tolist()                   
