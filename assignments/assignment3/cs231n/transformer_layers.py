...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        pos_index = torch.arange(0, max_len).unsqueeze(1)  # (max_len, 1)
        div_term = torch.exp(torch.arange(0, embed_dim, 2) * (-math.log(10000.0) / embed_dim))

        # Compute positional values sin/cos
        pe[0, :, 0::2] = torch.sin(pos_index * div_term)
        pe[0, :, 1::2] = torch.cos(pos_index * div_term)
    ...
    def forward(self, x):
        ...
        # 1. Add the positional encoding to the input x. 
        # We slice self.pe to match the actual sequence length S of the input.
        # Broadcasts over the batch dimension N automatically.
        output = x + self.pe[:, :S, :]
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        patches = x.reshape(
            N, C, H // self.patch_size, self.patch_size, W // self.patch_size, self.patch_size
        )
